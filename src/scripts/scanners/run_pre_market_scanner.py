...
Replicates TradingView "nk-pre-market-movers" screener:
  - US stocks
  - Price > $2
  - Market cap > $300M
  - Avg vol 30D > 500K
  - Pre-mkt change OUTSIDE -5% to +5%  (|chg| > 5%)
  - Pre-mkt volume > 100K

Run:  python run_pre_market_scanner.py
      python run_pre_market_scanner.py --limit 30 --min-pmchg 7
"""
...
DEFAULT_LIMIT      = 50
DEFAULT_MIN_PMCHG  = 5.0    # abs pre-mkt change threshold (%)
...
def run_scanner(
    ...
    print(f"\n{'─'*60}")
    print(f"  PRE-MARKET MOVERS SCANNER  |  {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    print(f"{'─'*60}")
    ...
    COLS = [
        ...
        "premarket_change", "premarket_volume", "sector", "type",
    ]
    BASE_FILTERS = [
        col("close") > min_price,
    ...
    df = pd.concat([
        _query(col("premarket_change") >  min_pmchg),
        _query(col("premarket_change") < -min_pmchg),
    ], ignore_index=True).drop_duplicates(subset="name")

    df = df.sort_values("premarket_change", ascending=False).head(limit)

    if df.empty:
        print("  No results found. Market may be closed / pre-market not active.\n")
        return df

    gainers = df[df["premarket_change"] > 0].copy()
    losers  = df[df["premarket_change"] < 0].copy()
    ...
    def print_section(subset, label):
        ...
        for i, (_, row) in enumerate(subset.iterrows(), 1):
            ...
            print(
                ...
                f"{fmt_vol(row['premarket_volume']):>9} "
...
def main():
    parser = argparse.ArgumentParser(description="Pre-Market Movers Scanner")
    parser.add_argument("--limit",      type=int,   default=DEFAULT_LIMIT,     help="Max results (default 50)")
    parser.add_argument("--min-pmchg",  type=float, default=DEFAULT_MIN_PMCHG, help="Min |PM change| %% (default 5)")
    ...
    if args.csv and not df.empty:
        fname = f"pre_movers_{datetime.now().strftime('%Y%m%d_%H%M')}.csv"
